...
import (
	...
	"github.com/MariusBobitiu/surface-lab/php-stack/tools/execute"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
	"google.golang.org/protobuf/types/known/structpb"
)

type phpStackServiceServer struct {
	UnimplementedPHPStackServiceServer
	logger *slog.Logger
}

func NewPHPStackServiceServer(logger *slog.Logger) PHPStackServiceServer {
	return &phpStackServiceServer{
		logger: logger,
	}
}

func (s *phpStackServiceServer) RunStack(ctx context.Context, req *RunStackRequest) (*RunStackResponse, error) {
	if req.GetTarget() == "" {
