...
import (
	...
	"github.com/MariusBobitiu/surface-lab/ecommerce-stack/tools/execute"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
	"google.golang.org/protobuf/types/known/structpb"
)

type shopifyStackServiceServer struct {
	UnimplementedShopifyStackServiceServer
	logger *slog.Logger
}

func NewShopifyStackServiceServer(logger *slog.Logger) ShopifyStackServiceServer {
	return &shopifyStackServiceServer{
		logger: logger,
	}
}

func (s *shopifyStackServiceServer) RunStack(ctx context.Context, req *RunStackRequest) (*RunStackResponse, error) {
	if req.GetTarget() == "" {
